...
using UnityEngine;
public class Slime : Champion
{
    //private static Slime instance = new Slime(300, 60, 37, 60, 37, 0.35f, 7, 100);
    private int mana;
    public Slime(float hp, float atk, float def, float mga, float mgd, float speed, float regen, int mana)
    {
    ...
    public override string QAttack(Champion target)
    {
        ...
        return "The Slime slapped you. Ouch!";
    }
    ...
    public override string WAttack(Champion target)
    {
        ...
        mana -= 50;
        target.SetStats(targetStats);
        return "The Slime prays. Somehow, its health is restored.";
    }
    ...
    //Checks for if Slime can use spells
